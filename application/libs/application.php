        ...
        if (file_exists('./application/controller/' . $this->url_controller . '.php')) {
            ...
            if (method_exists($this->url_controller, $this->url_action)) {
                ...
                if (isset($this->url_parameter_3)) {
                    ...
                    $this->url_controller->{$this->url_action}($this->url_action, $this->url_parameter_1, $this->url_parameter_2, $this->url_parameter_3);
                } elseif (isset($this->url_parameter_2)) {
                    // will translate to something like $this->home->method($param_1, $param_2);
                    $this->url_controller->{$this->url_action}($this->url_action, $this->url_parameter_1, $this->url_parameter_2);
                } elseif (isset($this->url_parameter_1)) {
                    // will translate to something like $this->home->method($param_1);
                    $this->url_controller->{$this->url_action}($this->url_action, $this->url_parameter_1);
                } else {
                    // if no parameters given, just call the method without parameters, like $this->home->method();
                    $this->url_controller->{$this->url_action}($this->url_action);
                }
            } else {
